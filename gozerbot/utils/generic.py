...
import time, sys, re, getopt, logging, types, os, random, socket, Queue 
...
def decodeperchar(txt, encoding='utf-8', what=""):
    ...
    if nogo:
        if what:
            logging.info("%s: can't decode %s characters to %s" % (what, nogo, encoding))
        else:
            logging.info("can't decode %s characters to %s" % (nogo, encoding))
    return u"".join(res)
